...
# Define the prompts for different sources
PROMPTS = {
    ...
    'medqa_train': "Your identity is a doctor, kindly provide answers to the medical questions with consideration of the patient's description. Answer with the best option directly.",
    'pubmedqa.ori_pqaa': "Your role as a doctor requires you to answer the medical questions taking into account the patient's description. Analyze the question given its context. Give both long answer and yes/no decision.",
    'umls': "As a medical professional, your responsibility is to address the medical questions using the patient's description. Explain the definition of the medical term. ",
    'umls_relation': "As a medical professional, your responsibility is to address the medical questions using the patient's description. Determine the relation between the two medical terms.",
    'chatdoc': "If you are a doctor, please answer the medical questions based on the patient's description."
}

# Process the dataset with or without truncating the options from input
def process_dataset(sampled_dataset, tokenizer, target_sample_size, remove_option, prompt):
    results = []
...
def main(args):
    ...
    initial_sample_size = int(args.sample_size * 3)
    ...
    # Select the appropriate prompt based on the source
    prompt = PROMPTS.get(args.source, "You are a medical professional. Answer the following question.")

    # Process the dataset until we get the desired number of valid samples
    results = process_dataset(sampled_dataset, tokenizer, args.sample_size, args.remove_option, prompt)
