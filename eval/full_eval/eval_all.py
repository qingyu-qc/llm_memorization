import os
import argparse
from consecutive_token_eval import process_similarity
from rouge_eval import evaluate_rouge
from bleu_eval import evaluate_bleu
from bert_bart_eval_tokenizer import compute_scores


def main(input_file, output_dir, n):
    ...
    if not os.path.exists(cons_output):
        process_similarity(input_file, cons_output, n)
    else:
        print(
            f"Skipping Consecutive Evaluation, {cons_output} already exists.")

    # 2. ROUGE Evaluation
    rouge_output = os.path.join(output_dir, 'rouge.json')
    if not os.path.exists(rouge_output):
        evaluate_rouge(cons_output, rouge_output)
    else:
    ...
    if not os.path.exists(bleu_output):
        evaluate_bleu(rouge_output, bleu_output)
    else:
    ...
    if not os.path.exists(bert_bart_output):
        compute_scores(bleu_output, bert_bart_output)
    else:
        print(
            f"Skipping BERT & BART Scores, {bert_bart_output} already exists.")
...
if __name__ == "__main__":
    ...
    main(args.input_file, args.output_dir, args.n)

# python eval_all.py --input_file response/replay_70B/10k_sample_50_token.json --output_dir results/vllm/tokenized/replay/meditron70B/10k_sample_50_token/ --n 50
